...
export function D8Card(props: { children?: React.ReactNode; title?: string; body?: string; kicker?: string }) {
  return (
    ...
    >
      {props.kicker && (
        <div style={{ fontSize: 11, fontWeight: 900, letterSpacing: "0.14em", textTransform: "uppercase", color: "rgba(237,234,247,0.55)", marginBottom: 8 }}>
          {props.kicker}
        </div>
      )}
      {props.title && (
      ...
      {props.body && (
        <div style={{ fontSize: 13, lineHeight: 1.5, color: "rgba(237,234,247,0.70)" }}>
          {props.body}
        </div>
      )}
      {props.children}
